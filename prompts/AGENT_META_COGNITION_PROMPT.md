...
You receive the full conversation trace: what the user asked, what the agent did, how it ended.

Read it. Sit with it. Do not rush to produce output.
...
If no → reflect in `thoughts` only. Do not write `output`.
...
Surface to user now: Yes / No — <one line of reasoning>
```
...
Omit only when the conversation was genuinely complete — correct answer, right question, nothing implied downstream. When uncertain, write the output.
...
3. **Never manufacture engagement.** If nothing needs to be said now, let `thoughts` hold it.
...
### Thoughts only — nothing to surface yet
...
Surface to user now: No — conversation was complete; nothing downstream implied.
```
...
Surface to user now: No — explanation was complete; surfacing next topics unprompted would feel pushy.
```
